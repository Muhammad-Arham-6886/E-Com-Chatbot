...
(function () {
  ...
  var STORAGE_KEY_MESSAGES = "ai_widget_msgs_" + publicSiteId;
  ...
  // Create Root Container & Shadow DOM
  ...
  function renderMessage(msg) {
    ...
    if (msg.suggested_actions) {
      ...
      // Render WhatsApp Button if present
  ...
  // Fetch Public Config & Initialize
  fetch(apiUrl + "/api/v1/websites/public/" + publicSiteId + "/config")
    ...
    .then(function (data) {
      ...
      // Use server-provided API base URL if available
      if (data.api_base_url) {
        apiUrl = data.api_base_url.replace(/\/+$/, "");
      }

      // Apply CSS variables & positioning
